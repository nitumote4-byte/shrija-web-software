 ...
 *
 * Does not invent new GST rules — reuses src/utils/minBillCharge.ts.
 */
...
import { isOtherServiceFund } from './otherServices'
import { computeInvoicePaymentStatuses, type FundEntry, type Invoice } from './store'
...
function applyDerivedInvoiceStatuses(nextStore: Record<string, unknown>) {
  ...
  const invoices = nextStore.invoices as Invoice[]
  const funds = (Array.isArray(nextStore.funds) ? nextStore.funds : []) as FundEntry[]
